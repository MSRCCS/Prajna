...
if (-Not $rootSrcFolder) {
	...
	Set-Variable -Name rootSrcFolder -Value '\\OneNet20\share\wezhu\PrajnaAzure' -Scope 1
}

if (-Not $localSrcFolder) {
	# Src folder that contains the latest PrajnaClient code in the local machine
	Set-Variable -Name localSrcFolder -Value 'C:\Code\p2plab\SkyNet' -Scope 1
}

if (-Not $targetSrcDir) {
	# Src folder that contains the latest PrajnaClient code in the remote machine
	Set-Variable -Name targetSrcDir -Value '\PrajnaSourceWezhu' -Scope 1
}

if (-Not $logdir) {
	# Log folder at remote machine
	Set-Variable -Name logdir -Value 'c:\OneNet\Log\wezhu' -Scope 1
}
...
if (-Not $homein) {
	...
	Set-Variable -Name homein -Value 'imhub-europe.cloudapp.net' -Scope 1
} 
...
if (-Not $CredentialFile) {
	...
	Set-Variable -Name CredentialFile -Value "C:\Users\wezhu\Documents\CredAzure" -Scope 1
}
